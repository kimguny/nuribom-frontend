...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#f4fbf7] shadow-sm">
      ...
      {/* 모바일 메뉴 */}
      {menuOpen && (
        <nav className="md:hidden bg-[#f4fbf7] border-t border-[#c8e6d5] px-6 py-4 flex flex-col gap-4">
          {NAV_ITEMS.map((item) => (
            <a
              key={item.href}
              href={item.href}
